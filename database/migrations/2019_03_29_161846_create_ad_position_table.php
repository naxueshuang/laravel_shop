...
class CreateAdPositionTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('jy_ad_position', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('positon_id')->default(0)->comment('广告位id');
            $table->string('ad_name',60)->default('')->comment('该条广告记录的广告名称');
            $table->string('image_url',150)->default('')->comment('广告稿图片地址');
            ...
            $table->integer('clicks')->default(rand(1,100))->comment('该广告点击数');
